...
#define define_generateCopy(Type) Type *generateCopy(void) { return new Type(*this); }
...
template<class Type> class LinkedListBase: public virtual ContainerBase
{
  public:
    ...
    LinkedListBase(LinkedListBase<Type> &);

    virtual ~LinkedListBase();

    LinkedListBase<Type> &operator=(LinkedListBase<Type> &);
...
{
  //cout << "LinkedListBase constructor\n";
...
template<class Type> LinkedListBase<Type>::LinkedListBase(LinkedListBase<Type> &that)
{
  //cout << "LinkedListBase copy constructor\n";

  ListItem *thisItem = &first, *thatItem = &that.first;

  first.prev = NULL;

  while (thatItem->next != &that.last)
  {
    thisItem->next = (ListItem*)(((Type*)(thatItem->next))->generateCopy());
  ...
  linkedListDebugMode = that.linkedListDebugMode;
...
{
  //cout << "LinkedListBase destructor\n";
...
template<class Type> LinkedListBase<Type> &LinkedListBase<Type>::operator=(LinkedListBase<Type> &b)
{
  //cout << "LinkedListBase operator=\n";

  free();

  ListItem *thisItem = &first, *thatItem = &b.first;

  first.prev = NULL;

  while (thatItem->next != &b.last)
  {
    thisItem->next = (ListItem*)(((Type*)(thatItem->next))->generateCopy());
    thisItem->next->prev = thisItem;
    thisItem = thisItem->next;
  ...
  linkedListDebugMode = b.linkedListDebugMode;
...
{
  if (this->n == 0 && first.next != &last) abort("free: fatal error!");
